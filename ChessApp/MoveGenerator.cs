...
namespace ChessApp
{
    internal class MoveGenerator
    {
        ...
        private static ulong PawnAttackRays(Side side, byte position)
        {
            if (side == Side.Black)
            {
                return blackPawnAttack[position];
            }
            else
            {
                return whitePawnAttack[position];
            }
        }
